...
use crate::{
    ...
    encryption::{
        random_nonce, CryptoError, DecryptedDeks, EncryptedDek, EncryptedObjIdent, MasterVersion,
    },
    id::BuiltinProp,
...
pub async fn load_cr_master_version(deps: &impl Db) -> Result<Option<MasterVersion>, CryptoError> {
    Ok(deps
        .query_map_opt(
...
pub async fn load_authly_instance(
    ...
    db: &impl Db,
    deks: &DecryptedDeks,
) -> Result<AuthlyInstance, CryptoError> {
...
async fn try_load_authly_id(
    deps: &impl Db,
    deks: &DecryptedDeks,
) -> Result<Option<AuthlyId>, CryptoError> {
    ...
    let private_key_der = PrivateKeyDer::try_from(private_key_plaintext)
        .map_err(|msg| CryptoError::Crypto(anyhow!("private key: {msg}")))?;
...
async fn load_certs(db: &impl Db) -> Result<Vec<AuthlyCert>, CryptoError> {
    Ok(db
        .query_filter_map(
...
async fn save_tls_cert<D: Db>(cert: &AuthlyCert, db: &D) -> Result<(), CryptoError> {
...
pub async fn list_all_cr_prop_deks(
    deps: &impl Db,
) -> Result<HashMap<BuiltinProp, EncryptedDek>, CryptoError> {
